...
- Antonis: Initial setup of the codebase built the environment in the cluster and executed the inferences for image and video benchmarks. Created scripts for evaluating the zero-shot performance of LLaMA-VId on Crime and animal-kingdom datasets, respectively. Further fine-tuning LLaMA-VID on the latter two new datasets. Substituted **gpt-3.5-turbo** with **Llama-3-8B-Instruct** to evaluate LLaMA-VID generated prediction for all QA tasks.
